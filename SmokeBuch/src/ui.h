...
bool do_button(SpriteID spriteID, IVec2 pos, int ID)
{
  ...
  IVec2 mousePosWorld = screen_to_ui(input->mousePos);
  ...
  if(is_active(ID))
  {
    if(key_released_this_frame(KEY_MOUSE_LEFT) &&
       point_in_rect(mousePosWorld, rect)) 
    {
      // Set inactive     
      uiState->active = {};
      return true;
  ...
  if(point_in_rect(mousePosWorld, rect))
  {
    set_hot(ID);
  }
...
bool do_button_extend(SpriteID spriteID, IVec2 pos, int ID, DrawData drawData = {})
{
  Sprite sprite = get_sprite(spriteID);
  IVec2 mousePosWorld = screen_to_ui(input->mousePos);
