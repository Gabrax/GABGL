...
void draw(float interpDT)
{
  ...
  switch(gameState->state)
  {
    ...
    {
      // Only draw UI
      renderData->clearColor = COLOR_BLACK;
      draw_text("Smokebuch",{-70, -150}, 2.0f);
...
void update_level(float dt)
{
  ...
  if(do_button(SPRITE_SAVE_BUTTON, {WORLD_WIDTH - 30, 12}, line_id(1)))
  {
...
void update()
{
  ...
  switch(gameState->state)
  {
    case GAME_STATE_MAIN_MENU:
    {
      int playbuttonID = line_id(1);  
      SpriteID PLAYsprite = SPRITE_PLAY_BUTTON;

      if(is_hot(playbuttonID)){
        PLAYsprite = SPRITE_SAVE_BUTTON;
      }
      if(do_button(PLAYsprite, {160, 90}, playbuttonID)){
        gameState->state = GAME_STATE_IN_LEVEL;
      }
     
      SpriteID somesprite = SPRITE_PLAY_BUTTON; 
      int buttonID = line_id(2);  
      if(is_hot(buttonID)){
        somesprite = SPRITE_SAVE_BUTTON;
      }
      if(do_button(somesprite, {160, 70}, buttonID)){
        gameState->state = GAME_STATE_IN_LEVEL;
      }
