...
// #############################################################################
void draw_text(char* text, Vec2 pos, float scale)
{
  ...
  while(char c = *(text++))
  {
    ...
    transform.size = { glyph.size.x * scale, glyph.size.y * scale };
    draw_quad(transform);
...
void draw_text_drop_shadow(char* text, Vec2 pos, float scale)
{
  draw_text(text, pos, scale);
  draw_text(text, pos - 1.0f, scale);
}
